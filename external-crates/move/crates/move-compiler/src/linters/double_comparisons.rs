    ...
    fn visit_exp_custom(&mut self, exp: &mut T::Exp) -> bool {
        if let UnannotatedExp_::BinopExp(lhs, sp!(_, BinOp_::Or), _, rhs) = &exp.exp.value {
            match (&lhs.exp.value, &rhs.exp.value) {
                (
                    UnannotatedExp_::BinopExp(lhs_inner, sp!(_, lhs_op), _, lhs_r),
                    UnannotatedExp_::BinopExp(rhs_inner, sp!(_, rhs_op), _, rhs_r),
                ) if are_expressions_comparable(lhs_inner, rhs_inner, lhs_r, rhs_r) => {
                    match (lhs_op, rhs_op) {
...
fn are_expressions_comparable(
    ...
    lhs_inner == rhs_inner && lhs_r == rhs_r
}
